...
*************************************************************************************/
u16 USART1_GetCmd_MODE1 (uint8_t *buff,u16 len)
{
	u16 getlen=0;//记录已经读取到的字节个数
	u16 want_num=8;//想要读取的数据个数，暂时只读取6位
	u16 i;//临时循环变量
	for (getlen=0;getlen<len;getlen++)
		buff[getlen]=0;
	if (RET_NUM>=10)
	{
//		delay_ms(10);//如果获取了一部分消息，等待数据接收完成
		getlen=0;
		...
		while(1)
		{
			...
			if (getlen==7)														//第4,5位决定数据长度
			{
				if (MY_MODEL==0)
				{
					if (buff[4]==0x00)								//第四位，决定是上位机还是系统运行的消息，
					{
						if (buff[5]==MY_STYLE)								//第五位，是不是发给自己的
						{
							if (buff[3]==0x03)
							{
								want_num=10;												//更新需要读取的数据个数
							}
						}
						else 									//不是发给自己的，退出
						{
							...
							if (buff[6]==0x00)
							{
								...
								else
								{
									buff[0]=0;
														...
														//关于上位机的消息，如果当前是在配置模式就接收上位机的消息
														//如果当前不是处于配置模式，就不接收上位机的消息
														//命令格式：
														//f0 fe df 06 10 style datalenh datalenl addr crc0 crc1
														//其中：style是要设置的控制器的类型，值只能是02 03 04之一
														//			数据长度，可以为1或2，第一位设置控制器地址，第二位如果是空调还要设置空调品牌
														//			addr是要设置的控制器的地址不能为0
														//			其他为固定值
				if (MY_MODEL==1)
				{
					if (buff[4]==0x10)					//上位机的消息
					{
						if (buff[5]!=MYSTYLE_KT) want_num=16;
						else want_num=16;
					}
					//上位机协议还没有定，暂时不写
				}
...
u16 USART1_GetCmd_MODE2_1 (uint8_t *buff,u16 len)
{
	...
	if (AUX)
	{
		...
		return getlen;
						...
						//和模式2的中断函数相配的，如果传输太频繁有BUG，不要用
u16 USART1_GetCmd_MODE2 (uint8_t *buff,u16 len)
{
	u16 getlen=0;//记录已经读取到的字节个数
	u16 i;//临时循环变量
	for (getlen=0;getlen<len;getlen++)
	...
	if (AUX&&(RET_NUM>7))
	{
		...
		if (RET_NUM==0) 
		{
			RX_START=0;							//如果全部读取了，重新开始接收
			USART1_RX_DISABLE;
			if (MODE2_IF_RECV>0) MODE2_IF_RECV--;
			RET_START=0;
			USART1_RX_ENABLE;
		}
	}
...
void RECV_MODE1_IRQ (void)
{
	...
	}else		//缓冲池不为空，标记为错误
	{
		USART_ClearFlag(USART1, USART_IT_RXNE);//没有执行读操作，需要程序中清除标志位
								...
								//有BUG，不要用
void RECV_MODE2_IRQ (void)
{
	static u8 old;
	static u8 now;
	static u8 type=0;//命令类型，1，温控系统的，2，上位机的
	static u16 want_length=8;
		old=now;
		now=(uint8_t)USART1->DR;//执行读操作自动清零中断标记
		
		if (type==0)//没在接收中
		{
//			if (MODE2_IF_RECV) return;
			
			if (old==0xff&&now==0xff)//如果数据头是温控系统的
			{
				RET_NUM=0;
				type=1;
				USART_RX_BUF[RET_NUM]=old;
				RET_NUM++;
				want_length=8;
			}
			else if (old==0xf0&&now==0xfe)//如果数据头是上位机的
			{
				RET_NUM=0;
				type=2;
				USART_RX_BUF[RET_NUM]=old;
		...
		if (type!=0)  //在接收过程中
		{
			USART_RX_BUF[RET_NUM]=now;
			RET_NUM++;
			if (type==1)
			{
				if (RET_NUM==8-1)
				{
					want_length=((old<<8)|now)+9;
				}
				if (want_length==RET_NUM)
				{
					MODE2_IF_RECV++;
					want_length=8;
					type=0;//结束传输
